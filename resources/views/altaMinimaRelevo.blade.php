<!doctype html>
    ...
    <body>
        ...
        <section class="altaNoticia">
            <form method="POST" action="{{route('altaMinimaRelevo')}}" id="altaNoticia" enctype="multipart/form-data">
                @csrf
                <div id="login" class="container">
                    <h2 style="color: white; text-align: center;">AÑADIR MINIMA RELEVO</h2>
                    <div class="form">
                        ...
                        <div class="item" style="text-align: center;">
                            ...
                            <select id="idUsuario2" name="idUsuario2" class="form-control">
                                @foreach ($nadadores as $nadador)
                                @if($nadador->estado===1)
                                    <option value="{{ $nadador->id }}" {{ old('idUsuario2') == $nadador->id ? 'selected' : '' }}>
                                        {{ $nadador->nombre }} {{ $nadador->primer_apellido }} {{ $nadador->segundo_apellido }}
                                    </option>
                                @endif
                                @endforeach
                        ...
                        <div class="item" style="text-align: center;">
                            ...
                            <select id="idUsuario3" name="idUsuario3" class="form-control">
                                @foreach ($nadadores as $nadador)
                                @if($nadador->estado===1)
                                    <option value="{{ $nadador->id }}" {{ old('idUsuario3') == $nadador->id ? 'selected' : '' }}>
                                        {{ $nadador->nombre }} {{ $nadador->primer_apellido }} {{ $nadador->segundo_apellido }}
                                    </option>
                                @endif
                                @endforeach
                        ...
                        <div class="item" style="text-align: center;">
                            ...
                            <select id="idUsuario4" name="idUsuario4" class="form-control">
                                @foreach ($nadadores as $nadador)
                                @if($nadador->estado===1)
                                    <option value="{{ $nadador->id }}" {{ old('idUsuario4') == $nadador->id ? 'selected' : '' }}>
                                        {{ $nadador->nombre }} {{ $nadador->primer_apellido }} {{ $nadador->segundo_apellido }}
                                    </option>
                                @endif
                                @endforeach
                        ...
                        <div class="item" style="text-align: center;">
                            ...
                            <select name="distancia" class="form-control">
                                <option value="4x50" {{ old('distancia') == '4x50' ? 'selected' : '' }}>4x50 metros</option>
                                <option value="4x100" {{ old('distancia') == '4x100' ? 'selected' : '' }}>4x100 metros</option>
                                <option value="4x200" {{ old('distancia') == '4x200' ? 'selected' : '' }}>4x200 metros</option>
                            </select>
                        </div><br>
