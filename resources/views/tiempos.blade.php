<!doctype html>
    ...
    <body>
            ...
            <div style="display: flex; justify-content: space-between; width: 100%;">
                <div style="width: 50%;">
                    <section class="altaNoticia">
                        <form method="POST" action="{{route('tiemposR')}}" id="altaNoticia" enctype="multipart/form-data">
                            @csrf
                                ...
                                <table class="table" style="background: #163b3d80; color:white; border-radius: 15px; margin-bottom: 20px;">
                                    <tbody>
                                        <tr style="border-radius: 15px;">
                                            ...
                                            <td style="background-color: transparent; color: white; border-radius: 15px; border-bottom: none; width: 25%; text-align: center; font-size: 1.2em; vertical-align: middle;" class="align-middle">
                                                    {{ $tiempo->tiempo }}
                                            </td>
                                            <td style="background-color: transparent; color: white; border-radius: 15px; border-bottom: none; width: 25%; text-align: center; font-size: 1.2em; vertical-align: middle;">{{ $tiempo->lugar }}</td>
                                            <td style="background-color: transparent; color: white; border-radius: 15px; border-bottom: none; width: 20%; text-align: center; font-size: 1.2em; vertical-align: middle;">
                ...
                <div style="width: 50%;">
                    <section class="altaNoticia" style="display: flex; justify-content: center; margin-bottom: 40px;">
                        <form method="POST" action="{{route('tiempos')}}" id="altaNoticia" enctype="multipart/form-data" style="width: 100%;">
                            @csrf
                            <div id="loginB" class="container">
                ...
                @if(isset($resultadosFinales))
                    @php
                        // Filtramos los resultados para obtener solo aquellos con usuarios activos
                    ...
                    @if($resultadosActivos->isEmpty())
                        <p style="background: #163b3d80; color:white; padding:1.5%; border-radius: 15px; font-size:120%; text-align:center;">No se encontraron tiempos.</p>
                    @else
                        @foreach($resultadosActivos as $tiempo)
                            @php
                                $nadador = collect($nadadores)->firstWhere('id', $tiempo->idUsuario1);
                            @endphp
                            @if($nadador)
                            <table class="table" style="background: #163b3d80; color:white; border-radius: 15px; margin-bottom: 20px;">
                                <tbody>
                                    <tr style="border-radius: 15px;">
                                        ...
                                        <td style="background-color: transparent; color: white; border-radius: 15px; border-bottom: none; width: 15%; text-align: center; font-size: 1.2em;">
                                            {{ $tiempo->tiempo }}
                                        </td>
                                        <td style="background-color: transparent; color: white; border-radius: 15px; border-bottom: none; width: 25%; text-align: center; font-size: 1.2em;">
                                            {{ $tiempo->lugar }}
                                        </td>
        ...
        <div style="display: flex; justify-content: space-between; width: 100%;">
            <div style="width: 50%;">
                <section class="altaNoticia">
                    <form method="POST" action="{{route('tiemposRN')}}" id="altaNoticia" enctype="multipart/form-data">
                        @csrf
                        <div id="loginB" class="container">
                            <h2 style="color: white; text-align: center;">BUSCAR TIEMPOS RELEVOS</h2>
                            <div class="form" style="display: flex; justify-content: space-between;">
                ...
                <div style="width: 90%; margin: 0 auto;">
                    @if(isset($tiemposRelevosNad))
                    @if($tiemposRelevosNad->isEmpty())
                        <p style="background: #163b3d80; color:white; padding:1.5%; border-radius: 15px; font-size:120%; text-align:center;">No se encontraron tiempos.</p>
                    @else
                        @foreach($tiemposRelevosNad as $tiempo)
                            @php
        ...
        <div style="width: 50%;">
            <section class="altaNoticia">
                <form method="POST" action="{{route('tiemposN')}}" id="altaNoticia" enctype="multipart/form-data">
                    @csrf
                    <div id="loginB" class="container">
                        <h2 style="color: white; text-align: center;">BUSCAR TIEMPOS</h2>
                        <div class="form">
            ...
            <div style="width: 90%; margin: 0 auto;">
                @if(isset($tiempos))
                    @php
                        // Filtramos los tiempos para obtener solo aquellos del usuario actual
                        $tiemposUsuarioActual = $tiempos->filter(function($tiempo) use ($nadadores) {
                            $nadador = $nadadores->firstWhere('id', $tiempo->idUsuario1);
                            return $nadador && $nadador->id == Auth::user()->id;
                        });
                    @endphp

                    @if($tiemposUsuarioActual->isEmpty())
                        <p style="background: #163b3d80; color:white; padding:1.5%; border-radius: 15px; font-size:120%; text-align:center;">No se encontraron tiempos.</p>
                    @else
                        @foreach($tiemposUsuarioActual as $tiempo)
                            @php
                                $nadador = $nadadores->firstWhere('id', $tiempo->idUsuario1);
                            @endphp
                            @if($nadador && $nadador->id == Auth::user()->id)
                                <table class="table" style="background: #163b3d80; color:white; border-radius: 15px; margin-bottom: 20px;">
                                    <tbody>
                                        <tr style="border-radius: 15px;">
                                            <td style="background-color: transparent; color: white; border-radius: 15px; border-bottom: none; width: 40%; text-align: center; font-size: 1.2em;">
